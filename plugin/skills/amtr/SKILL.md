...
Thin wrapper over `amtr recall Handoff` and `amtr key`. Compaction inside one
...
2. Run exactly one command:
   ...
   PATH="$PATH:$HOME/.local/bin:$HOME/.cargo/bin:/opt/homebrew/bin" \
     amtr recall Handoff --amtr-key "<amtr_key>"
   ```
   ...
   `amtr` reads the receiving session id from `CLAUDE_CODE_SESSION_ID` or
   `CODEX_THREAD_ID`. If neither is set it refuses the move. Report that to the
   user instead of retrying — the host did not tell this session what it is
   called, and nothing you can type here fixes that.
