...
//! Hooks are spawned by the host with no guaranteed environment, so a tunable
//! like XDG_DATA_HOME could resolve differently across invocations and present
//! as memory loss. The rule is hardcoded: an existing store wins, then the
//! presence of `~/.local` decides.
//!
//! The home directory itself is unavoidably environmental: `home_dir()` reads
//! `$HOME` and falls back to the passwd entry. Every state transition now runs
//! in this binary, so there is only one resolver to keep correct.
...
impl Store {
    ...
    /// Opens the resolved store without creating anything. `peek` uses this so
    /// observing a machine that has never synthesized does not create state.
    pub fn open_existing() -> io::Result<Option<Store>> {
    ...
    pub(crate) fn cortex(&self) -> PathBuf {
        self.base.join(CORTEX)
    }

    pub(crate) fn row_path(&self, session_id: &str) -> PathBuf {
        self.base
            .join(CORTEX)
    ...
    pub(crate) fn deadline_path(&self, session_id: &str) -> PathBuf {
        self.base
            .join(CORTEX)
            .join(format!("{}.deliver-deadline", slug(session_id)))
    }
    ...
    /// Every readable row, in deterministic session order.
    pub fn rows(&self) -> io::Result<Vec<Row>> {
        let mut rows = Vec::new();
        let entries = match fs::read_dir(self.cortex()) {
            Ok(entries) => entries,
            Err(error) if error.kind() == io::ErrorKind::NotFound => return Ok(rows),
        ...
        for entry in entries {
            ...
            let raw = match fs::read_to_string(&path) {
                Ok(raw) => raw,
                Err(error) => {
                    eprintln!("{}: cannot read {}: {error}", now(), path.display());
                    continue;
                }
            };
            match serde_json::from_str::<Row>(&raw) {
                Ok(row) => rows.push(row),
                Err(error) => eprintln!("{}: {} is not readable: {error}", now(), path.display()),
            }
        }
    ...
    /// Test-only convenience. Runtime reads participate in atomic claims and
    /// therefore live in `recall`, not as a non-claiming store getter.
    #[cfg(test)]
...
/// Validates the host-minted identifier accepted at both hook boundaries.
///
/// Keeping this policy beside `slug` makes the relationship explicit: hook
/// payloads reject anything outside the host's ASCII alphabet, while slugging
/// remains defence in depth for explicit handoffs and migrated rows.
pub fn validate_session_id(session_id: &str) -> io::Result<()> {
            ...
            .all(|byte| byte.is_ascii_alphanumeric() || matches!(byte, b'.' | b'_' | b'-'))
    {
        ...
        Err(io::Error::new(
            ...
            "host payload has an invalid session_id",
        ))
    }
}
...
///
/// Substitution is per byte so every non-ASCII byte becomes an underscore and
/// no Unicode normalization or platform-specific filename interpretation can
/// change the result. Hook payloads accept the host's ASCII id alphabet; this
/// remains defensive for explicit and migrated rows.
...
mod tests {
    ...
    #[test]
    fn listing_keeps_readable_rows_when_another_row_is_corrupt() {
        let store = scratch();
        let readable = row("readable", Some("amtr-readable"), "state");
        store.save(&readable).unwrap();
        fs::write(store.cortex().join("broken.json"), "not json").unwrap();

        assert_eq!(store.rows().unwrap(), vec![readable]);
    }
    ...
    #[test]
    fn base_directory_does_not_move_when_local_appears_later() {
        let root = scratch_dir();
        let cases: [(&[&str], &str); 5] = [
            (&[], ".amtr"),
        ...
        for (i, (existing, expected)) in cases.iter().enumerate() {
            ...
            assert_eq!(
                Store::base_dir_under(&home),
                home.join(expected),
                "wrong directory for a home containing {existing:?}"
            );
        }
    }
